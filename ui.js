class UI {
  ...
  static createCol(row, id, desc, value) {
  ...
  static displayTask(task) {
    ...
    UI.createCol(newRow, task.id, 'name', task.name);
    // these values will bet set when taskChanged() is called below
    UI.createCol(newRow, task.id, 'logged', '');
    UI.createCol(newRow, task.id, 'last-active', '');
